...
export exePR=${useP}"parsec-benchmark/pkgs/kernels/canneal/"
cd ${exePR}

export exeP=${exePR}"inst/amd64-linux.gcc/bin/canneal"

${useP}${repo}/build/X86/gem5.opt \
-d ${useP}"gem5-hetero-garnet/scripts/PARSEC/m5out/" \
${useP}${repo}/configs/my/se.py \
--cmd=${exeP} \
--option="1 15000 2000 200000.nets 64" \
--ruby \
--router-latency=3 \
--link-latency=27 \
--num-cpus=${core_num} \
