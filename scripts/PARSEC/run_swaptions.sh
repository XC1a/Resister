...
export exePR=${useP}"parsec-benchmark/pkgs/apps/swaptions/"
cd ${exePR}

export exeP=${exePR}"inst/amd64-linux.gcc/bin/swaptions"

${useP}${repo}/build/X86/gem5.opt \
-d ${useP}"gem5-hetero-garnet/scripts/PARSEC/m5out/" \
${useP}${repo}/configs/my/se.py \
--cmd=${exeP} \
--option="-ns 32 -sm 20000 -nt 1" \
--ruby \
--router-latency=3 \
--link-latency=27 \
--num-cpus=${core_num} \
