...
export exePR=${useP}"parsec-benchmark/pkgs/apps/blackscholes/"
cd ${exePR}

export exeP=${exePR}"inst/amd64-linux.gcc/bin/blackscholes"

${useP}${repo}/build/X86/gem5.opt \
-d ${useP}"gem5-hetero-garnet/scripts/PARSEC/m5out/" \
${useP}${repo}/configs/my/se.py \
--cmd=${exeP} \
--option="1 ./input_simmedium/in_16K.txt prices.txt" \
--ruby \
--router-latency=3 \
--link-latency=27 \
--num-cpus=${core_num} \
