...
export exeP="../../build/build_base_mytest-m64.0000/imagick_r"
...
cd ${useP}cpu2017/benchspec/CPU/538.imagick_r/run/run_base_refrate_mytest-m64.0000

${useP}${repo}/build/X86/gem5.opt \
-d ${useP}"gem5-hetero-garnet/scripts/SPEC/m5out/" \
${useP}${repo}/configs/my/se.py \
--cmd=${exeP} \
--option="-limit disk 0 refrate_input.tga -edge 41 -resample 181% -emboss 31 -colorspace YUV -mean-shift 19x19+15% -resize 30% refrate_output.tga" \
--ruby \
--router-latency=3 \
--link-latency=27 \
--num-cpus=${core_num} \
