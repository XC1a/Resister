...
unsigned int lcg_rand(unsigned int seed)
{
    ...
    const unsigned int adder = 1013904223;
    seed = multiplier * seed + adder;
    return seed;
}

/// trick to creat different enviorment
int trick()
{
  int a[4096+32];
  int b = 0;
  unsigned int seed = lcg_rand(RND);
  for (size_t i = 0; i < (32+(seed%4096)); i++)
  {
    // a[((47*RND)+i*3)%4096] = i;
    a[i] = i;
    b = b | a[((seed)+i*7)%4096];
        ...
        while (c)
          {
            ...
            // int b[64/4];
            // b[0]= 0xabcd1234;
            volatile unsigned char *FlushBuffer = malloc(32*1024);
            for (size_t i = 0; i < 32*1024; i += 64)
              FlushBuffer[i] = i;
            unsigned long stamp = __rdtsc();
            //note the following instruction may be fetched and decoded by branch prediction
            ...
            /////////// xr wang for test, just modify the judgement conditions
            // if ( (mpi_limb_signed_t)e < 0 )
            if (1)
              {
